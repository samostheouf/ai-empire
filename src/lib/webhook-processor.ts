...
export async function processStripeWebhookEvent(stripeEvent: Record<string, unknown>): Promise<void> {
  ...
  switch (evtType) {
    case 'checkout.session.completed': {
      ...
      if (templateId && email) {
        const dbResult = await safeQuery(async () => {
          ...
          // Transactional upsert on the unique sessionId — prevents duplicate
          // orders under concurrent webhook deliveries.
          ...
          if (order.createdAt.getTime() < Date.now() - 60_000) {
            // Pre-existing order (idempotent replay): skip side effects.
    ...
    case 'customer.subscription.deleted': {
      const subscription = evtObject;
      const customerId = subscription.customer;
      logger.info('webhook', `Subscription deleted: ${subscription.id}, status: ${subscription.status}`);
      await safeQuery(async () => {
        const { prisma } = await import('@/lib/db');
    ...
    html: `
      ...
      <body style="font-family: -apple-system, sans-serif; background: #f8fafc; padding: 40px 20px;">
        <div style="max-width: 500px; margin: 0 auto; background: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
          ...
          <div style="padding: 32px;">
            <h2 style="color: #1e293b; margin: 0 0 16px;">Merci pour votre achat de ${escapeHtml(templateName)} !</h2>
