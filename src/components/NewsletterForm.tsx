  ...
  if (status === 'success') {
    return (
      <div className="rounded-2xl border border-green-800/50 bg-green-900/20 p-8 text-center">
        ...
        <h3 className="mt-3 text-lg font-semibold text-white">Merci pour votre inscription !</h3>
        <p className="mt-2 text-indigo-300">Vous recevrez nos prochains articles directement dans votre boîte mail.</p>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-2xl border border-indigo-800/50 bg-indigo-900/30 p-8">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="text-lg font-semibold text-white">Newsletter NeuraAPI</h3>
          <p className="text-sm text-indigo-300">Articles exclusifs, conseils IA et offres spéciales.</p>
        </div>
      </div>
      <form onSubmit={handleSubmit} className="flex gap-3 mt-4">
          <label htmlFor="newsletter-form-email" className="sr-only">Email newsletter</label>
          <input
      ...
      <p className="mt-3 text-xs text-indigo-400">Pas de spam. Désabonnement en un clic. Max 2 emails/mois.</p>
    </div>
  )
}
